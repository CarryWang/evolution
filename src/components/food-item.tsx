...
export default function FoodItem({ food, onClick }: FoodItemProps) {
  ...
  return (
    ...
    >
      ...
      {/* 代币名称标签 - 使用类型守卫确保类型安全 */}
      {isTokenFood(food) && (
        <div className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 bg-black/70 text-white text-xs px-2 py-0.5 rounded-full whitespace-nowrap">
          {food.tokenName}
          {/* {!food.isBeingEaten && (
            <span className="ml-1 text-xs text-green-400">点击关闭</span>
          )} */}
        </div>
      )}
